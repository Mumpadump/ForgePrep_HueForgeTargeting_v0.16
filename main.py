...
from ui.luminance_histogram import LuminanceHistogramWindow
...
class ForgePrepApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.luminance_analysis: LuminanceAnalysis | None = None
    ...
    def build_window(self) -> None:
        ...
        ttk.Label(
            main,
            text=f"ForgePrep v{APP_VERSION} — Luminance Manufacturing",
        ...
        ttk.Button(
            toolbar,
            text="Build Luminance Map",
            command=self.build_luminance_map,
        ).pack(side="left", padx=8)

        ttk.Button(
            toolbar,
            text="View Histogram",
            command=self.open_luminance_histogram,
        ).pack(side="left", padx=8)
        ...
        rows_container = ttk.Frame(palette_frame)
        ...
        self.rows_frame = ttk.Frame(self.palette_canvas)

        self.rows_window = self.palette_canvas.create_window(
            (0, 0),
        ...
        self.rows_frame.bind(
            "<Configure>",
            self._update_palette_scroll_region,
        )
        self.palette_canvas.bind(
            "<Configure>",
            self._resize_palette_rows,
        )
        ...
        self.palette_canvas.bind(
            "<Enter>",
            self._bind_palette_mousewheel,
        )
        self.palette_canvas.bind(
            "<Leave>",
            self._unbind_palette_mousewheel,
        )
    ...
    def _update_palette_scroll_region(self, event=None) -> None:
        self.palette_canvas.configure(
            scrollregion=self.palette_canvas.bbox("all")
        )

    def _resize_palette_rows(self, event) -> None:
        self.palette_canvas.itemconfigure(
            self.rows_window,
            width=event.width,
        )

    def _bind_palette_mousewheel(self, event=None) -> None:
        self.root.bind_all(
            "<MouseWheel>",
            self._scroll_palette,
        )

    def _unbind_palette_mousewheel(self, event=None) -> None:
        self.root.unbind_all("<MouseWheel>")

    def _scroll_palette(self, event) -> None:
        if event.delta == 0:
            return

        direction = -1 if event.delta > 0 else 1
        self.palette_canvas.yview_scroll(direction, "units")
    ...
    def build_luminance_map(self) -> None:
        """Build manufacturing levels from image brightness."""
        if self.original_image is None:
            messagebox.showinfo("No image", "Open an image first.")
            return

        try:
            level_count = int(self.color_count_var.get())
            self.luminance_analysis = analyze_luminance(
                self.original_image,
                level_count,
            )

            # Use neutral grayscale display colors for the existing palette UI.
            old_layers = self.layers
            new_layers: list[PaletteLayer] = []

            for level in self.luminance_analysis.levels:
                filament = (
                    old_layers[level.index].filament
                    if level.index < len(old_layers)
                    else ""
                )
                gray = (level.center, level.center, level.center)
                new_layers.append(
                    PaletteLayer(
            ...
            self.layers = new_layers
            self.assignment_map = self.luminance_analysis.level_map.copy()
            self.original_assignment_map = self.assignment_map.copy()
            self.reduced_image = (
                self.luminance_analysis.create_grayscale_preview()
            )
            self.analysis = []
            self.manufacturing_map = None
            self.selected_mask_index = min(
            ...
            self.show_luminance_report()

            self.cleanup_summary_var.set(
                "Luminance map generated. Cleanup has not been applied."
            )
            self.status_var.set(
                f"{level_count}-level luminance map generated."
            )

        except Exception as error:
            messagebox.showerror(
                "Luminance analysis failed",
                str(error),
            )

    def show_luminance_report(self) -> None:
        """Display brightness ranges and coverage for each level."""
        if self.luminance_analysis is None:
            return

        total_pixels = self.luminance_analysis.level_map.size
        levels = self.luminance_analysis.levels

        separation_scores = [
            levels[index + 1].center - levels[index].center
            for index in range(len(levels) - 1)
        ]
        minimum_separation = (
            min(separation_scores)
            if separation_scores
            else 255
        )

        if minimum_separation >= 35:
            rating = "Excellent"
        elif minimum_separation >= 24:
            rating = "Good"
        elif minimum_separation >= 14:
            rating = "Moderate"
        else:
            rating = "Weak"

        lines = [
            "LUMINANCE MANUFACTURING ANALYSIS",
            "",
            f"Image size: {self.original_image.width} × "
            f"{self.original_image.height} px",
            f"Detected levels: {len(levels)}",
            f"Minimum neighboring separation: {minimum_separation}",
            f"Overall separation rating: {rating}",
            "",
            "LEVEL DETAILS — DARKEST TO BRIGHTEST",
            "",
        ]

        for level in levels:
            percentage = (
                level.pixel_count / total_pixels * 100
                if total_pixels
                else 0
            )
            lines.append(
                f"{level.display_name:<10} "
                f"center {level.center:>3}  "
                f"range {level.minimum:>3}–{level.maximum:<3}  "
                f"{percentage:>6.2f}%"
            )
        ...
        lines.extend(
            [
                "",
                "Use each row's Preview button to inspect that "
                "luminance family as a mask.",
            ]
        )
    ...
    def open_luminance_histogram(self) -> None:
        if self.luminance_analysis is None:
            messagebox.showinfo(
                "No luminance map",
                "Open an image and click Build Luminance Map first.",
            )
            return

        LuminanceHistogramWindow(
            self.root,
            self.luminance_analysis,
        )
